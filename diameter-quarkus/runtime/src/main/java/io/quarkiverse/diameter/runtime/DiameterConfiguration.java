...
public class DiameterConfiguration extends EmptyConfiguration
{
	...
	{
		AppConfiguration config = EmptyConfiguration.getInstance().add(RealmName, realm.realmName()).add(RealmHosts, realm.peers()).add(RealmLocalAction, realm.localAction().name()).add(RealmEntryIsDynamic, realm.dynamic()).add(RealmEntryExpTime, realm.expTime());
		if (realm.applicationId().isPresent()) {
			config.add(ApplicationId, addApplicationID(realm.applicationId().get()));
		}
