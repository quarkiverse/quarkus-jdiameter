...
class DiameterProcessor
{
	...
	@Record(RUNTIME_INIT)
	@BuildStep
	ServiceStartBuildItem generateDiameterConfiguration(DiameterRecorder recorder, DiameterConfigs diameterConfig, ShutdownContextBuildItem shutdownContextBuildItem, TlsRegistryBuildItem tlsRegistryBuildItem, BuildProducer<SyntheticBeanBuildItem> syntheticBeanBuildItemBuildProducer)
	{
		if (diameterConfig.diameterConfigs() != null) {
			for (String configName : diameterConfig.diameterConfigs().keySet()) {
				if (Boolean.TRUE.equals(diameterConfig.diameterConfigs().get(configName).enabled())) {
					syntheticBeanBuildItemBuildProducer.produce(createSyntheticBean(configName, Configuration.class, DOTNAME_CONFIGURATION, configName.equals(DiameterConfig.DEFAULT_CONFIG_NAME)).createWith(recorder.diameterConfiguration(tlsRegistryBuildItem.registry(), configName)).done());

					syntheticBeanBuildItemBuildProducer.produce(createSyntheticBean(configName, Stack.class, DOTNAME_STACK, configName.equals(DiameterConfig.DEFAULT_CONFIG_NAME)).createWith(recorder.diameterStack(shutdownContextBuildItem, tlsRegistryBuildItem.registry(), configName)).done());
				}//if
			}//for
		}//if
		...
		for (AnnotationInstance annotation : index.getIndex().getAnnotations(DOTNAME_DIAMETER_SERVICE)) {
			if (!annotation.target().asClass().name().equals(DOTNAME_DIAMETER_SERVICE_INTERCEPTOR)) {
				if (annotation.target().annotation(DOTNAME_DIAMETER_SERVICE_OPTIONS) == null) {
					errors.produce(new ValidationPhaseBuildItem.ValidationErrorBuildItem(new DiameterSetupException("Missing @DiameterServiceOptions annotation on class " + annotation.target().asClass().name().withoutPackagePrefix())));
				}
			}
		}
	}
	...
	@BuildStep
	void declareDiameterServicesAsBean(CombinedIndexBuildItem index, BuildProducer<AdditionalBeanBuildItem> additionalBeans)
	{
		List<String> diameterServices = index.getIndex().getKnownClasses().stream().filter(ci -> ci.hasAnnotation(DiameterService.class) && !ci.hasAnnotation(Interceptor.class)).map(ci -> ci.name().toString()).collect(Collectors.toList());

		additionalBeans.produce(new AdditionalBeanBuildItem.Builder().addBeanClasses(diameterServices).setUnremovable().setDefaultScope(DotNames.APPLICATION_SCOPED).build());
	}
	...
	private static <T> SyntheticBeanBuildItem.ExtendedBeanConfigurator createSyntheticBean(String clientName, Class<T> type, DotName exposedType, boolean isDefaultConfig)
	{
		...
		if (isDefaultConfig) {
			...
			configurator.addQualifier(Default.class).addQualifier(DiameterConfig.class);
		}
		else {
			configurator.addQualifier().annotation(DiameterConfig.class).addValue("value", clientName).done();
		}
