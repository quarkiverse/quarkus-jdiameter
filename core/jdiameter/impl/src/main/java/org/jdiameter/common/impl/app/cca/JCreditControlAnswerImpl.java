...
public class JCreditControlAnswerImpl extends AppAnswerEventImpl implements JCreditControlAnswer
{
	...
	{
		Avp validityAvp = super.message.getAvps().getAvp(VALIDITY_TIME_AVP_CODE);
		if (validityAvp == null) {
			Avp mscc = super.message.getAvps().getAvp(Avp.MULTIPLE_SERVICES_CREDIT_CONTROL);
			try {
				validityAvp = mscc != null ? mscc.getGrouped().getAvp(Avp.VALIDITY_TIME) : null;
			}
