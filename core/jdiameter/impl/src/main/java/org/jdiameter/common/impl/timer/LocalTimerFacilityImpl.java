...
public class LocalTimerFacilityImpl implements ITimerFacility {
    ...
    private final GenericObjectPool pool = new GenericObjectPool(new TimerTaskHandleFactory(), 100000,
            GenericObjectPool.WHEN_EXHAUSTED_GROW, 10, 20000);
    ...
    public void cancel(Serializable f) {
        if (f != null && f instanceof TimerTaskHandle) {
            ...
            if (timerTaskHandle.future != null) {
                logger.debug("Cancelling timer with id [{}] and delay [{}]", timerTaskHandle.id,
                        timerTaskHandle.future.getDelay(TimeUnit.MILLISECONDS));
