...
public class TLSTransportClient {
     ...
     */
    public TLSTransportClient(TLSClientConnection parenConnection, IConcurrentFactory concurrentFactory,
    ...
    void sendMessage(IMessage message) throws IOException, AvpDataException, NotInitializedException, ParseException {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("About to send a byte buffer of size [{}] over the TLS socket [{}]", messageBuffer.array().length,
                    socketDescription);
        }
        lock.lock();
        try {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Sent a byte buffer of size [{}] over the TLS nio socket [{}]", messageBuffer.array().length,
                    socketDescription);
        }
    }
    ...
    private boolean isSuccess(IMessage message) throws AvpDataException {
        ...
        if (resultAvp == null) {
            ...
            if (resultAvp == null) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Discarding message since SSL handshake has not been performed on [{}], dropped message [{}]. No result type avp.",
            ...
            if (resultAvp == null) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Discarding message since SSL handshake has not been performed on [{}], dropped message [{}]. No result avp.",
                            socketDescription, message);
                }
            }
        }
