...
public class ServerCCASessionImpl extends AppCCASessionImpl
	implements ServerCCASession, NetworkReqListener, EventListener<Request, Answer>
{
	...
	public ServerCCASessionImpl(IServerCCASessionData data, ICCAMessageFactory fct, ISessionFactory sf,
				...
				case IDLE:
					switch (eventType) {
						...
						case RECEIVED_TERMINATE:
							Answer errorAnswer = ((Request) localEvent.getRequest().getMessage())
								.createAnswer(ResultCode.UNKNOWN_SESSION_ID);
							session.send(errorAnswer);
							logger.debug(
								"Received an UPDATE or TERMINATE for a new session. Answering with 5002 (UNKNOWN_SESSION_ID) and terminating session.");
							// and let it throw exception anyway ...
						default:
							throw new InternalException("Wrong state: " + IDLE + " on event: " + eventType
							                            + " " + localEvent.getRequest() + " " +
	...
	private void startTcc(Avp validityAvp)
	{
		...
		if (sessionData.getTccTimerId() != null) {
			...
			this.sessionData
				.setTccTimerId(super.timerFacility.schedule(this.getSessionId(), TCC_TIMER_NAME, tccTimeout * 1000));
	...
	public void onTimer(String timerName)
	{
		if (timerName.equals(TCC_TIMER_NAME)) {
			logger.debug("Received onTimer for TCC timer, session ID '{}'", getSessionId());
			...
			try {
				...
				switch (request.getCommandCode()) {
					case ReAuthRequest.code:
						handleEvent(new Event(Event.Type.RECEIVED_RAA, factory.createReAuthRequest(request),
							factory.createReAuthAnswer(answer)));
						break;
					default:
