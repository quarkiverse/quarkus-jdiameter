...
/**
 * Basic implementation of ShServerSession - can be one time - for UDR, PUR and constant for SNR-PNR pair, in case when
 * SNA contains response code from range different than 2001-2004(success codes) user is responsible for maintaing state
 * - releasing etc, same goes if result code is contained Experimental-Result AVP <br>
 * If ShSession moves to
 * ShSessionState.TERMINATED - it means that no further messages can be received via it and it should be discarded.
 * <br>
 * <br>
 *
...
public class ShServerSessionImpl extends ShSession
    ...
    public ShServerSessionImpl(IShServerSessionData sessionData, IShMessageFactory fct, ISessionFactory sf,
        ...
        try {
            ...
            switch ((Event.Type) localEvent.getType()) {
                ...
                case RECEIVE_PUSH_NOTIFICATION_ANSWER:
                    listener.doPushNotificationAnswerEvent(this, (PushNotificationRequest) localEvent.getRequest(),
                            (PushNotificationAnswer) localEvent.getAnswer());
                    break;
                case RECEIVE_SUBSCRIBE_NOTIFICATIONS_REQUEST:
                    listener.doSubscribeNotificationsRequestEvent(this,
                            (SubscribeNotificationsRequest) localEvent.getRequest());
                    break;
            ...
            try {
                if (request.getApplicationId() == appId) {
                    if (request.getCommandCode() == SubscribeNotificationsRequest.code) {
                        handleEvent(new Event(Event.Type.RECEIVE_SUBSCRIBE_NOTIFICATIONS_REQUEST,
                                factory.createSubscribeNotificationsRequest(request), null));
                    } else if (request.getCommandCode() == UserDataRequest.code) {
                        handleEvent(
                                new Event(Event.Type.RECEIVE_USER_DATA_REQUEST, factory.createUserDataRequest(request), null));
                    } else if (request.getCommandCode() == ProfileUpdateRequest.code) {
                        handleEvent(new Event(Event.Type.RECEIVE_PROFILE_UPDATE_REQUEST,
                                factory.createProfileUpdateRequest(request), null));
                    } else {
    ...
    private class AnswerDelivery implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (request.getApplicationId() == appId) {
                    if (request.getCommandCode() == PushNotificationRequest.code) {
                        handleEvent(new Event(Event.Type.RECEIVE_PUSH_NOTIFICATION_ANSWER,
                                factory.createPushNotificationRequest(request),
                    ...
                    logger.warn("Message with Application-Id {} reached Application Session with Application-Id {}. Skipping.",
