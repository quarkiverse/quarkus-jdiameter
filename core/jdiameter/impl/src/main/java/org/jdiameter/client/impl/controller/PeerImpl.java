...
public class PeerImpl extends AbstractPeer implements IPeer {
    ...
    public PeerImpl(final PeerTableImpl table, int rating, URI remotePeer, String ip, String portRange, IMetaData metaData,
        ...
        this(table, rating, remotePeer, ip, portRange, metaData, config, peerConfig, fsmFactory, trFactory, parser,
                statisticFactory, concurrentFactory, null,
                sessionDataSource);
    }
        ...
        if (connection == null) {
            ...
            if (portRange != null) {
                try {
                    ...
                    logger.debug(
                            "Selecting local port randomly from range '{}-{}'. Doing {} tries (some ports may not be tested, others tested more than once).",
    ...
    public String toString() {
        return "CPeer{" + "Uri=" + uri + "; State=" + (fsm != null ? fsm.getState(PeerState.class) : "n/a") + "; Con="
                + connection + "}";
    }
    ...
    protected class ActionContext implements IContext {
        ...
        @Override
        public String toString() {
            return new StringBuilder("ActionContext [getPeerDescription()=").append(getPeerDescription())
                    .append(", isConnected()=").append(isConnected()).append(", isRestoreConnection()=")
                    .append(isRestoreConnection()).append("]").toString();
        }
                    ...
                    if (table instanceof MutablePeerTableImpl) { // available only to server, client skip this step
                        ...
                        if (peerTable.isDuplicateProtection()) {
                            ...
                            if (originInfo != null) {
                                ...
                                peerTable.saveToDuplicate(
                                        message.getDuplicationKey(originInfo[0], message.getEndToEndIdentifier()), message);
                            }
                        }
                    }
                }
