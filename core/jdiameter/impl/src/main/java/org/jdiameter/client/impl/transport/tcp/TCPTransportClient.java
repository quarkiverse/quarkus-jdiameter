...
public class TCPTransportClient implements Runnable {
    ...
    public void initialize() throws IOException, NotInitializedException {
        logger.debug("Initialising TCPTransportClient. Origin address is [{}] and destination address is [{}]", origAddress,
                destAddress);
        if (destAddress == null) {
    ...
    public void run() {
        ...
        int sleepTime = 250;
        logger.debug("Sleeping for {}ms before starting transport so that listeners can all be added and ready for messages",
                sleepTime);
        try {
    ...
    public void sendMessage(ByteBuffer bytes) throws IOException {
        if (logger.isDebugEnabled()) {
            logger.debug("About to send a byte buffer of size [{}] over the TCP nio socket [{}]", bytes.array().length,
                    socketDescription);
        }
        int rc = 0;
        // PCB - removed locking
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Sent a byte buffer of size [{}] over the TCP nio socket [{}]", bytes.array().length,
                    socketDescription);
        }
    }
    ...
    private boolean seekMessage() {
        ...
        try {
            ...
            if (storage.limit() < messageLength) {
                ...
                logger.debug("Received partial message, waiting for remaining (expected: {} bytes, got {} bytes).",
                        messageLength, storage.position());
                return false;
            }
