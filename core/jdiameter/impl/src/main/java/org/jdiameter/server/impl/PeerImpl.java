            ...
            try {
                if (connection == null || !connection.isConnected()) {
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "Connection is null or not connected. Looking for one in incConnections with key [{}]. Here are the incConnections :",
                                key);
        ...
        public boolean receiveMessage(IMessage message)
        {
            ...
            if (message.isRequest()) {
                ...
                if (!realmTable.realmExists(destRealm)) {
                    ...
                    logger.warn(
                            "Received a request for an unrecognized realm: [{}]. Answering with 3003 (DIAMETER_REALM_NOT_SERVED) Result-Code.",
                            destRealm);
