...
public class PeerFSMImpl implements IStateMachine {
    ...
    public PeerFSMImpl(IContext aContext, IConcurrentFactory concurrentFactory, Configuration config,
    ...
    private void runQueueProcessing() {
        try {
            ...
            final IStatisticRecord messagePrcAverageTime = statisticFactory.newCounterRecord(
                    ...
                    new IStatisticRecord.DoubleValueHolder() {
                        ...
                        public double getValueAsDouble() {
                            ...
                            IStatisticRecord mpta = queueStat
                                    .getRecordByName(IStatisticRecord.Counters.MessageProcessingTime.name());
            ...
            queueStat = statisticFactory.newStatistic(context.getPeerDescription(), IStatistic.Groups.PeerFSM, queueSize,
                    messagePrcAverageTime);
    ...
    public boolean handleEvent(StateEvent event) throws InternalError, OverloadException {
        ...
        try {
            if (logger.isDebugEnabled()) {
                logger.debug("Placing event [{}] into linked blocking queue with remaining capacity: [{}].", event,
                        ...
                        public boolean processEvent(StateEvent event) {
                            switch (event.encodeType(EventTypes.class)) {
                                ...
                                case SEND_MSG_EVENT:
                                    ...
                                    break;
                                default:
                                    logger.debug("Unknown event type: {} in state {}", event.encodeType(EventTypes.class),
                                            state);
                                    return false;
                            }
                            return true;
                    ...
                    new MyState() { // INITIAL
                        ...
                        public boolean processEvent(StateEvent event) {
                            switch (event.encodeType(EventTypes.class)) {
                                ...
                                case CEA_EVENT:
                                    clearTimer();
                                    if (context.processCeaMessage(((FsmEvent) event).getKey(),
                                            ((FsmEvent) event).getMessage())) {
                                        switchToNextState(FsmState.OKAY);
                                    } else {
                                        doDisconnect();
                                        setTimer(REC_TIMEOUT);
